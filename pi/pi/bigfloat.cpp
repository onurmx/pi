...
BigFloat BigFloat::mul(std::vector<std::vector<my_complex>>& twiddle_table, const BigFloat& x, size_t p, int tds) const {
	//  Multiplication
	...
	fft_forward(twiddle_table, Ta.get(), k, tds);            //  Transform 1st operand
	fft_forward(twiddle_table, Tb.get(), k, tds);            //  Transform 2nd operand
	fft_pointwise(Ta.get(), Tb.get(), k);     //  Pointwise multiply
	fft_inverse(twiddle_table, Ta.get(), k, tds);            //  Perform inverse transform.
...
BigFloat BigFloat::rcp(std::vector<std::vector<my_complex>>& twiddle_table, size_t p, int tds) const {
	...
	BigFloat T = rcp(twiddle_table, s, tds);

	//  r1 = r0 - (r0 * x - 1) * r0
	return T.sub(this->mul(twiddle_table, T, p, tds).sub(BigFloat(1), p).mul(twiddle_table, T, p, tds), p);
}

BigFloat BigFloat::div(std::vector<std::vector<my_complex>>& twiddle_table, const BigFloat& x, size_t p, int tds) const {
	//  Division
	return this->mul(twiddle_table, x.rcp(twiddle_table, p, tds), p, tds);
}

BigFloat invsqrt(std::vector<std::vector<my_complex>>& twiddle_table, uint32_t x, size_t p, int tds) {
	...
	//  Recurse at half the precision
	BigFloat T = invsqrt(twiddle_table, x, s, tds);

	BigFloat temp = T.mul(twiddle_table, T, p);     //  r0^2
	temp = temp.mul(twiddle_table, x, p, tds);       //  r0^2 * x
	temp = temp.sub(BigFloat(1), p); //  r0^2 * x - 1
	temp = temp.mul(500000000);         //  (r0^2 * x - 1) / 2
	temp.exp--;
	temp = temp.mul(twiddle_table, T, p, tds);       //  (r0^2 * x - 1) / 2 * r0
